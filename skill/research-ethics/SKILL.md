...
description: 为中国医学研究伦理申报和国家医学研究登记准备“研究计划书 → 用户确认 → 按平台顺序可复制填写稿”。当用户需要根据研究计划书、伦理材料或资助材料生成逐项填写清单、ChiCTR 中英文配对内容、确认缺口或相关附件清单时使用。当前 V1 只支持研究者发起的观察性研究；干预性和产品注册路线必须明确转为 V2，不得臆造。
---
...
1. 从用户提供的研究计划书、伦理材料和资助材料中提取候选值；在对话中注明依据位置。不要把未被材料支持的内容写成事实。若材料含真实题目、机构、人员、联系方式或编号，先创建脱敏副本，再将副本作为可保留的测试输入。
2. 先确定路线：观察性研究、诊断试验是/否、公开策略。若路线在 V2 或被阻止，停止生成并解释边界。
3. 按 [references/intake-schema.md](references/intake-schema.md) 建立脱敏 intake。将会影响可见字段的选项写入 `selections`，文本、日期、数值和附件准备状态写入 `values`，可重复组写入 `repeat_groups`。
4. 只向用户询问计划书无法决定、平台必须填写或会改变本页结构的事项。把每个红色“待用户确认”明确分为：真实事实待确认、平台规则待核验、计划书可提取但映射待补全、平台实时字典待选择。把“账户自动带入”“实时字典中选择”“需上传附件”分别标明，不伪造内容。
5. 运行 `scripts/render_copyable_checklist.py` 生成按平台页面顺序的 Markdown 填写稿。它会显示证据等级；`待用户确认` 不是可直接提交的值。
6. 使用工作区依赖中的 Python 运行 `scripts/render_copyable_docx.py`，由同一 Markdown 生成 Word 填写稿。所有 Word 填写稿必须使用同一视觉语义：建议填写／选择的实际值为蓝色；`待用户确认` 为红色；字段名、可选项、来源、操作和规则证据均为黑色。生成后按 documents 工作流渲染并检查页面；若渲染环境缺少 LibreOffice，则执行 DOCX 结构检查并明确说明未完成视觉检查。
...
- 不保留网页源码、截图、Cookie、密码、令牌、真实账号资料或用户未要求保存的研究材料。
